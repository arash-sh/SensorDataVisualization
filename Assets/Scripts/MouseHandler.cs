...
/// <summary>
/// This class can be added as component to every game object that has to react to mouse input.
/// After adding the component, a handler function and the respective type (down, up, etc.) are added to the respective lists. On every mouse event, the handlers that listen to the respective event are invoked.Currently, only handlers without input areguments are possible
/// </summary>

public class MouseHandler : MonoBehaviour {

    public enum MOUSE_EVENT { DOWN, OVER, ENTER, EXIT, BUTTON};
    private List<MOUSE_EVENT> Types = new List<MOUSE_EVENT>(); 
    private List<UnityAction> Handlers = new List<UnityAction>(); 

    public void SetHandler(UnityAction handler, MOUSE_EVENT type)
    {
    ...
    void OnMouseDown()
    {
         for (int i = 0; i< Types.Count; i++)
            if (Types[i] == MOUSE_EVENT.DOWN)
                Handlers[i].Invoke();
    }
    //void OnMouseOver()
    //{
    //    for (int i = 0; i< Types.Count; i++)
    //        if (Types[i] == MOUSE_EVENT.OVER)
    //            Handlers[i].Invoke();  
    //}
    void OnMouseEnter()
    {
        for (int i = 0; i< Types.Count; i++)
            if (Types[i] == MOUSE_EVENT.ENTER)
                Handlers[i].Invoke();  
    }
    void OnMouseExit()
    {
        for (int i = 0; i < Types.Count; i++)
            if (Types[i] == MOUSE_EVENT.EXIT)
                Handlers[i].Invoke();
    }
    void OnMouseUpAsButton()
    {
        for (int i = 0; i < Types.Count; i++)
            if (Types[i] == MOUSE_EVENT.BUTTON)
                Handlers[i].Invoke();
    }

    // static void DummyHandler()
    // {
    //     Debug.LogError("Handler for mouse interaction not set");
    // }
